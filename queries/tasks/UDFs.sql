...
CREATE OR REPLACE FUNCTION analytics.fn_age_group(
	p_age INT
)
RETURNS TEXT
LANGUAGE sql
...
-- Օրինակ 2․ Հաճախորդի «ստաժի» (tenure) դասակարգում
...
-- Օրինակ 3․ Ապրանքի գնի մակարդակի դասակարգում
...
SELECT
	...
	product_name,
	price,
	analytics.fn_price_tier(price) AS price_titer
FROM analytics.products;



-- Օրինակ 4․ Պատվերի չափի դասակարգում
...
-- Օրինակ 5․ Պատվերի ակտիվության կարգավիճակի նորմալացում
...
-- TABLE-VALUED UDF
--Օրինակ 1․ Վերջին պատվերները հաճախորդի համար
...
--Օրինակ 2․ Ամենաեկամտաբեր ապրանքներ
...
-- Օրինակ 3․ Հաճախորդներ ըստ քաղաքի
...
-- Օրինակ 4․ Պատվերի ապրանքներ՝ մանրամասն տվյալներով
...
-- Stored Procedures
-- Օրինակ 1․ Ապրանքի ավելացում կամ թարմացում (Upsert Product)
...
-- Օրինակ 2․ Պատվերի կարգավիճակի թարմացում
...
-- Օրինակ 3․ Հաճախորդի ջնջում՝ անվտանգության ստուգմամբ
...
CALL analytics.sp_delete_customer(1);


-- Օրինակ 4․ Պատվերի ընդհանուր գումարների թարմացում
...
-- VIEWES
-- Հաճախորդների դայմենշն View (Customer Dimension View)
...
-- Օրինակ 3․ Ապրանքների եկամտի ամփոփ View
...
-- Օրինակ 4․ Ակտիվ պատվերների View
...
SELECT *
FROM analytics.mv_customers_geography;
